 ...
 * - 以 token 摘要作键而非原文：即便 Redis 数据被读取，也无法反推出可用凭证；
 * - 过期交给 Redis 原生 TTL（到期自动删除），替代此前 JSON 文件方案的遍历清理；
...
final class TokenStore
{
    ...
    /**
     * 校验 token，返回 subject（如 "json:2" / "orm:3"）；无效或过期返回 null。
     */
