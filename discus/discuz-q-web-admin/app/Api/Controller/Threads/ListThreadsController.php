...
class ListThreadsController extends AbstractListController
{
    ...
    protected function data(ServerRequestInterface $request, Document $document)
    {
        ...
        // 圈子帖子ID
        $quan = Arr::get($request->getQueryParams(), 'quan');
        // 课程帖子ID
        $ke = Arr::get($request->getQueryParams(), 'ke');
        if (!(
        ...
        $threads = $this->search($actor, $filter, $sort, $limit, $offset, $page, $quan, $ke);
     ...
     * @return Collection
     */
    public function search($actor, $filter, $sort, $limit = null, $offset = 0, $page, $quan, $ke)
    {
        /** @var Builder $query */
        ...
        $this->applyFilters($query, $filter, $actor, $quan, $ke);
     ...
     * @param User $actor
     */
    private function applyFilters(Builder $query, array $filter, User $actor, $quan, $ke)
    {
        ...
        //圈子帖子id
        if($quan >0) {
            $query->where('threads.quanid', $quan);
        } else {
            $query->where('threads.quanid', NULL);
        }
        //课程帖子id
        if($ke >0) {
            $query->where('threads.keid', $ke);
        } else {
            $query->where('threads.keid', NULL);
        }
        ...
        // 海报帖
        if ($isPoster = Arr::get($filter, 'isPoster')) {
            if ($isPoster == 'yes') {
                $query->where('threads.is_poster', true);
            } elseif ($isPoster == 'no') {
                $query->where('threads.is_poster', false);
            }
        }

        // 关闭评论
        if ($isPinglun = Arr::get($filter, 'isPinglun')) {
            if ($isPinglun == 'yes') {
                $query->where('threads.is_pinglun', true);
            } elseif ($isPinglun == 'no') {
                $query->where('threads.is_pinglun', false);
            }
        }
        
        // 原创
        if ($isOriginal = Arr::get($filter, 'isOriginal')) {
            if ($isOriginal == 'yes') {
                $query->where('threads.is_original', true);
            } elseif ($isOriginal == 'no') {
                $query->where('threads.is_original', false);
            }
        }
