...
class ResourceThreadV2Controller extends DzqController
{
    ...
    public $include = [
        ...
        'user:id,username,avatar,follow_count as followCount,updated_at as updatedAt,last_login_ip as lastLoginIp,register_ip as registerIp',
    ...
    public function main()
    {
        ...
        */
        ...
        $ip = $data['author']['lastLoginIp'];
        if (empty($ip)){
            $ip = $data['author']['registerIp'];
        }
        $info = $this->Qqwry->getlocation($ip);
        if (isset($this->appcode)){
            $data['author']['province'] = $info;
        }
    ...
    public function parseContent($post, $request){
        ...
        $attachments = $post->images
            ...
            ->map(function (Attachment $attachment) use ($attachmentSerializer) {
                if ($attachment->type === Attachment::TYPE_OF_IMAGE || $attachment->type === Attachment::TYPE_OF_HAIBAO) {
