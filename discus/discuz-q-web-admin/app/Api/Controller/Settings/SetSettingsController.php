...
class SetSettingsController implements RequestHandlerInterface
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        $settings->each(function ($setting) {
            ...
            if ($key == 'site_manage' || $key == 'site_growup' || $key == 'site_ask' || $key == 'site_htmlapp' || $key == 'site_weiapp' ||  $key == 'site_weiapp' || $key == 'site_growups' || $key == 'site_integral' || $key == 'site_signinday' || $key == 'site_signin' || $key == 'api_freq') {
                if (is_array($value)) {
            ...
            if ($key == 'site_create_thread14' && $value == 0) {
                $permission = 'createThread.14';
                Permission::query()->where('permission', 'like', "$permission%")->delete();
            }
