...
export const time2MinuteOrHour = time => {
  const now = (process.client && window.currentTime) || new Date();
  const pass = new Date(time);
  const result = (now - pass) / 1000;

  if (parseInt(result) === 0) {
    return '刚刚发布';
  }
  // 分钟差小于1分钟
  if (parseInt(result) < 60) {
    return `${parseInt(result)}秒前`;
  }
  // 分钟差小于60分钟
  if (parseInt(result / 60) < 60) {
    return `${parseInt(result / 60)}分钟前`;
  }
  // 小时差小于16小时
  if (parseInt(result / 60 / 60) < 16) {
    return `${parseInt(result / 60 / 60)}小时前`;
  }
  // 超过16个小时展示 年月日时分
  return time
    .replace(/T/, ' ')
