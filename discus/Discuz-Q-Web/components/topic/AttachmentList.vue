<template>
  <div
    ...
    @click.self="downloadAttachment(file.url, file.fileName)"
  >
    <div class="info">
      <div>
        <svg-icon
          :type="extensionValidate(file.extension)"
      ...
      <span
        class="size"
      >{{
        parseInt((file.fileSize / 1024).toString()).toLocaleString()
      }}
        KB</span>
    </div>
    <div class="action">
      <div v-if="!unpaid">
        <span
          v-if="canReview"
          class="download"
          @click.self="preview(file.url)"
        >{{ $t("post.preview") }}</span>
        <span
          class="download"
          @click.self="downloadAttachment(file.url, file.fileName)"
        >{{ $t("post.download") }}</span>
      </div>
    ...
    >
      <div class="dialog-cont">
        <img :src="previewData.image" alt="" class="img">
      </div>
      <div slot="footer" class="pagination">
        <el-button
          :class="{ hidden: previewPage === 1 }"
          type="primary"
          size="medium"
          @click="prev"
        >{{ $t("post.prev") }}</el-button>
        <div class="pages">{{ previewPage }} / {{ previewTotal }}</div>
        <el-button
          :class="{ hidden: parseInt(previewPage) >= parseInt(previewTotal) }"
          size="medium"
          type="primary"
          @click="next"
        >{{ $t("post.next") }}</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    unpaid() {
      return (
        (parseFloat(this.price) > 0 && !this.isPaid)
        || (parseFloat(this.attachmentPrice) > 0 && !this.isPaidAttachment)
      );
    },
    canReview() {
      return (
        ...
        && ((parseFloat(this.price) === 0
          && parseFloat(this.attachmentPrice) > 0
          && this.isPaidAttachment)
          || (parseFloat(this.price) > 0 && this.isPaid))
        && this.file.isRemote
      );
    }
