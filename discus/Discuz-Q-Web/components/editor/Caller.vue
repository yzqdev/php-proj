  ...
  >
    <div class="top">
      <div class="container-selectedCaller">
        <div
          v-for="(username, index) in selectedFriends"
          :key="index"
          ...
          <ul v-infinite-scroll="load" infinite-scroll-disabled="disabled">
            <li
              ...
              :class="{
                ...
                selected: selectedFriends.indexOf(user.username) >= 0
              }"
      ...
      <div class="friends">
        ...
        <div v-loading="friendsLoading" class="container-caller">
          <div
            ...
            @click="
              selectedFriends.indexOf(user.username) >= 0
                ? selectedFriends.splice(
                  selectedFriends.indexOf(user.username),
                  1
                )
                : selectedFriends.push(user.username)
            "
          >
    ...
    <div class="bottom">
      <div class="container">
        ...
        <el-button
          ...
          size="medium"
          type="primary"
          @click="$emit('selectedCaller', formatValue())"
...
export default {
  ...
  methods: {
    getFriends() {
      ...
      return this.$store.dispatch('jv/get', ['follow', { params }]).then(
        data => {
    ...
    selectFriendFromUl(user) {
      this.selectedFriends.indexOf(user.username) < 0
        && this.selectedFriends.push(user.username);
      this.clear();
    },
    clear() {
