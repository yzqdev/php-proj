<template>
  <div class="post-container" :style="{ padding: padding }">
    ...
    <div class="main-cont">
      ...
      <!-- 问答贴 -->
      <div v-if="item.type === 5" class="question-user">
        <!-- 悬赏问答贴 -->
        <div v-if="!item.question">
          {{ $t("topic.questionsAndAnswers") }}
        ...
        <!-- 未回答 -->
        <div v-if="item.question && item.question.is_answer === 0" class="inline">
          <div class="inline blue">
            ￥{{
              item &&
                item.questionTypeAndMoney
      ...
      <template v-if="item.firstPost">
        <div class="first-post" @click.self="toDetail">
          <a
            :href="`/thread/${item._jv && item._jv.id}`"
