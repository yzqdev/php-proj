  ...
  >
    <div class="top">
      <div class="container-selectedCaller">
        <div
          v-show="Object.keys(selectedFriend).length > 0"
          class="selectedCaller"
        >
          <span>@ {{ selectedFriend.username }}</span>
          <svg-icon
            style="font-size: 13px; cursor: pointer; fill: #6d6d6d; margin-left: 5px"
            type="close"
            @click="selectedFriend = {}"
          />
        </div>
      </div>
      ...
      <div class="container-search-list">
        <label>
          <input
            ref="inputCaller"
            :placeholder="$t('post.pleaseInput')"
            type="text"
            class="input-caller"
            @input="searchUser"
          >
          ...
          <ul v-infinite-scroll="load" infinite-scroll-disabled="disabled">
            <li
              ...
              :class="{
                ...
                selected: selectedFriend.username === user.username
              }"
      ...
      <div class="friends">
        ...
        <div class="container-user-list">
          <div
            ref="preQaUser"
            v-loading="preQaUsers.loading"
            class="container-caller prePage"
          >
            <div
              v-for="(user, index) in preQaUsers.list"
              :key="index"
              :class="{
                caller: true,
                selected: selectedFriend.username === user.username
          ...
          <div
            ref="currentQaUser"
            v-loading="currentQaUsers.loading"
            class="container-caller currentPage"
          >
            <div
              v-for="(user, index) in currentQaUsers.list"
              :key="index"
              :class="{
                caller: true,
                selected: selectedFriend.username === user.username
          ...
          <div
            ref="nextQaUser"
            v-loading="nextQaUsers.loading"
            class="container-caller nextPage"
          >
            <div
              v-for="(user, index) in nextQaUsers.list"
              :key="index"
              :class="{
                caller: true,
                selected: selectedFriend.username === user.username
          ...
          <div class="pagination">
            ...
            <svg-icon
              :class="{
                'page-button': true,
                'next-button': true,
                disabled: pageNumberPagination >= totalPage
              }"
              type="drop-down"
              @click="nextPage"
            />
    ...
    <div class="bottom">
      <div class="container">
        <el-button
          :disabled="Object.keys(selectedFriend).length === 0"
          size="medium"
          type="primary"
          @click="$emit('selectedQaCaller', selectedFriend)"
        >{{ $t("post.confirmChoice") }}</el-button>
      </div>
    </div>
  </message-box>
</template>
...
export default {
  ...
  methods: {
    initUserList() {
      if (this.pageNumberPagination === 1) {
        // 第一页
      ...
      } else if (this.pageNumberPagination === (this.totalPage || 10)) {
        // 最后一页
    ...
    getUserList(page, obj) {
      ...
      return this.$store.dispatch('jv/get', ['/users', { params }]).then(
        data => {
          // TODO 响应页数判断
          const {
            _jv: {
    ...
    getSearchList() {
      ...
      this.$store.dispatch('jv/get', ['users', { params }]).then(
        data => {
          const {
            _jv: {
              json: { meta }
