<template>
  <div class="audio">
    ...
    <div
      v-else-if="currentFile && currentAudio.isPlay"
      class="control"
      @click="$emit('pause')"
    >
    ...
    <div class="info">
      <div class="title">
        ...
        <span v-if="currentFile && playing" class="duration-audio">
          {{ formatCurrentTime(currentAudio.currentTime) }} /
          {{ formatDuration(currentAudio.duration) }}
        </span>
        <span v-else class="duration-audio">--:--</span>
      </div>
      <div
        v-if="currentFile && playing"
        id="progress"
        ref="progress"
        class="progress"
      >
        <div
          :style="{ width: convertToPercentage() + '%' }"
          class="progress-item"
        />
        <div
          id="control-ball"
...
export default {
  ...
  computed: {
    currentFile() {
      return this.currentAudio.id === this.file.pid;
    },
    playing() {
  ...
  methods: {
    ...
    formatCurrentTime(currentTime) {
      ...
      const m = current_minute < 10 ? `0${current_minute}` : current_minute;
      const s = current_seconds < 10 ? `0${current_seconds}` : current_seconds;
      return `${m}:${s}`;
    },
    convertToPercentage() {
