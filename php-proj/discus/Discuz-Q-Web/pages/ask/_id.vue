  ...
  <div class="container">
    ...
    <main class="cont-center">
      <div class="cont-center-head">
        <indexask-filter
          :show="forums && forums.other && forums.other.can_create_thread_ask"
          :show-default="false"
          @onChangeFilter="onChangeFilter"
          @onChangeType="onChangeType"
          @onChangeSort="onChangeSort"
    ...
    <aside class="cont-left">
      <div class="category background-color">
        <categoryask-pays :list="categoryData" @onChange="onChangeCategory" />
      </div>
    </aside>
...
export default {
  ...
  async asyncData({ store, params, query }, callback) {
    ...
    try {
      ...
      const categoryData = await apiRequest.readCategories({ params: categoryParams });
      /*IFTRUE_default*/
  ...
  methods: {
    ...
    onChangeCategory(ids) {
      if (ids.id !== 0) {
        this.$router.push({
          path: `/ask/${ids.id}?url=asks`,
          query: { search_ids: ids.search_ids }
        });
      } else {
        this.$router.push('/asks');
      }
    },
    // 筛选
    onChangeFilter(val) {
