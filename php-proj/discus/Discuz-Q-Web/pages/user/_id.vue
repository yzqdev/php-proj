<template>
  <div v-loading="loading" class="profile">
    ...
    <div v-if="userInfo && forums" class="profile-h">
      ...
      /*FITRUE_default*/
      /*IFTRUE_pay*/
      <Avatar :user="userInfo" :size="80" :round="true" class="avatar" />
      /*FITRUE_pay*/
      <div class="profile-info">
        <span class="">
          <div class="">
            <img
              :src="userInfo.growup.growupimageUrl"
              class=""
            >
          </div>
...
export default {
  ...
  methods: {
    ask() {
      ...
      this.$router.push(`/thread/postpay?type=5&beaskId=${this.userId}`);
    },
    handleScroll() {
      const scrollTop
        = window.pageYOffset
...
@import "@/assets/css/pay_scss/user_id.scss";
/*FITRUE_pay*/
.isFixed {
  position: fixed;
  top: 0;
...
::v-deep.el-tabs {
  ...
  .el-tabs__header .el-tabs__item:nth-child(3) {
    padding-left: 20px;
  }
    .el-tabs__header .el-tabs__item:nth-child(4) {
    padding-left: 9px;
  }
  .el-tabs__header .el-tabs__item.is-active {
    color: black;
