...
export default {
  ...
  computed: {
    ...
    siteRegMode() {
      return this.$store.state.site.info.attributes.set_reg.register_type || {};
    }
  },
  watch: {
    userId(id) {
  ...
  methods: {
    ...
    tologin() {
      // 根据几种不同的登录模式跳转至不同页面
      switch (this.siteRegMode) {
        case REGISTER_MODE.USER:
          this.$router.push(`/user/login?preurl=${this.preurl}`);
          break;
        case REGISTER_MODE.MOBILE:
          this.$router.push(
            `/user/phone-login-register?code=${this.code}&preurl=${this.preurl}`
          );
          break;
        case REGISTER_MODE.NOSENCE:
          this.$router.push(
            `/user/wechat?code=${this.code}&preurl=${this.preurl}`
          );
          break;
        default:
          this.$router.push(`/user/login?preurl=${this.preurl}`);
    ...
    loadThreads() {
      ...
      const params = {
        ...
        sort: '-createdAt',
        include:
          'user,user.groups,firstPost,firstPost.images,firstPost.postGoods'
          + ',category,threadVideo,threadAudio,question,question.beUser,question.beUser.groups',
        'page[number]': 1,
        'page[limit]': 10,
        'filter[isApproved]': 1,
        'filter[isSite]': 'yes'
