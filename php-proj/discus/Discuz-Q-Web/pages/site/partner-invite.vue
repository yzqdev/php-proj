  ...
  methods: {
    ...
    check2() {
      ...
      if (this.inviteCode && this.inviteCode.length !== 32) {
        if (!this.inviteData.id) {
          ...
          this.codeTitle = this.$t('site.join') + this.forums.set_site.site_name || 'Discuz! Q';
    ...
    loadThreads() {
      ...
      const params = {
        'filter[isDeleted]': 'no',
        sort: '-createdAt',
        include: 'user,user.groups,firstPost,firstPost.images,firstPost.postGoods,category'
          + ',threadVideo,threadAudio,question,question.beUser,question.beUser.groups',
        'page[number]': 1,
        'page[limit]': 10,
        'filter[isApproved]': 1,
        'filter[isSite]': 'yes'
