      ...
      <template v-for="(item, index) in favoriteList">
        ...
        >
          <div slot="bottom-right" class="delete-container">
            ...
            ><svg-icon type="delete" />{{ $t("topic.delete") }}</span>
          </div>
        </post-item>
        <post-item v-else :key="index" :item="item" :show-share="false" padding="20px 35px 32px 30px">
          <div slot="bottom-right" class="delete-container">
            <span
              class="delete"
              @click="handleFavorite(item, index)"
        ...
        >
          <template slot="bottom-right">
            <span
              class="btn delete"
              @click="handleFavorite(item, index)"
            ><svg-icon type="delete" class="icon" />{{ $t("topic.delete") }}</span>
          </template>
        </post-item-pay>
        <post-item-pay v-else :key="index" :item="item" :show-share="false">
          <template slot="bottom-right">
            <span
              class="btn delete"
              @click="handleFavorite(item, index)"
...
export default {
  ...
  methods: {
    getFavoriteList() {
      ...
      this.$store
        .dispatch('jv/get', ['favorites', { params }])
        .then(
          data => {
            ...
            if (data._jv) {
              this.hasMore
                = this.favoriteList.length < data._jv.json.meta.threadCount;
            }
          },
          e => {
            this.handleError(e);
    ...
    handleFavorite(item, index) {
      ...
      this.$confirm(
        this.$t('topic.confirmCancelCollection'),
...
@import "@/assets/css/pay_scss/favorite.scss";
/*FITRUE_pay*/
</style>
