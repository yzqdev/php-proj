...
export default {
  data: function() {
    return {
      ...
      selectList: {
        ...
        "createThread.14":[], //发布提问帖扩展项
        'viewThreads': [], // 查看主题列表扩展
        'viewaskThreads': [], // 查看提问主题列表扩展
        'createThread':[], // 发布主题扩展项
        'createaskThread':[], // 发布问答主题扩展项
        'thread.ask':[], // 发布提问展项
        'thread.reply':[], // 回复主题扩展项
        'thread.askreply':[], // 回复提问主题扩展项
        'thread.viewPosts':[], // 查看主题详情扩展
        'thread.viewaskPosts':[], // 查看提问主题详情扩展
        'thread.canBeReward': [], //打赏扩展
        'thread.canaskBeReward': [], //打赏扩展
        'thread.edit':[], // 编辑主题扩展
        'thread.hide':[], // 删除主题扩展
        'thread.essence':[], // 加精扩展
        ...
        'thread.editOriginalThreadOrPost': [], // 编辑自己的原创主题
        'thread.hideOwnThreadOrPost': [], // 删除自己的主题、回复
        'thread.askedit':[], // 提问编辑主题扩展
        'thread.askhide':[], // 提问删除主题扩展
        'thread.askessence':[], // 提问加精扩展
        'thread.editaskPosts':[], // 提问编辑回复扩展
        'thread.hideaskPosts':[], // 提问删除回复扩展
        'thread.editaskOwnThreadOrPost': [], // 提问编辑自己的主题、回复
        'thread.hideaskOwnThreadOrPost': [], // 提问删除自己的主题、回复
        'thread.freeViewPosts.1':[],
        'thread.freeViewPosts.2':[],
        'thread.freeViewPosts.3':[],
        'thread.freeViewPosts.4':[],
        'thread.freeViewPosts.5':[],
        'thread.freeaskViewPosts.1':[], // 提问
        'thread.freeaskViewPosts.2':[],
        'thread.freeaskViewPosts.3':[],
        'thread.freeaskViewPosts.4':[],
        'thread.freeaskViewPosts.5':[],
      },
      activeTab: {
        // 设置权限当前项
        title: "操作权限",
      ...
      pubFunc:{
        ...
        "createThread.14.disabled":false,
      },
      // 扩展全选
      expandItem: [
        'thread.viewPosts',
        ...
        'thread.editOriginalThreadOrPost',
        'thread.hideOwnThreadOrPost', //
        'thread.askedit', // 提问
        'thread.askhide', // 提问
        'thread.askessence', // 提问
        'thread.editaskPosts', // 提问
        'thread.hideaskPosts', // 提问
        'thread.editaskOwnThreadOrPost', // 提问
        'thread.hideaskOwnThreadOrPost', // 提问
        'thread.freeViewPosts.1',
        'thread.freeViewPosts.2',
        'thread.freeViewPosts.3',
        'thread.freeViewPosts.4',
        'thread.freeViewPosts.5',
        'thread.freeaskViewPosts.1',
        'thread.freeaskViewPosts.2',
        'thread.freeaskViewPosts.3',
        'thread.freeaskViewPosts.4',
        'thread.freeaskViewPosts.5'
  ...
  methods: {
    ...
    getData() {
      Promise.all([this.getCategories(), this.getCategoriesask(), this.getGroupResource(), this.getSiteInfo()])
        .then(
          res => {
            this.handleCategories(res[0]);
            this.handleCategorieseask(res[1]);
            this.handleGroupResource(res[2]);
            this.signUpSet(res[3]);
    ...
    handleCategorieseask(res) {
      if (res.errors) return this.$message.error(res.errors[0].code);

      this.categoriesaskList = [{ id: "", name: "全局", children: [] }]
      res.readdata.forEach(item => {
        this.mapCategoryId.set(parseInt(item._data.id), item._data.parentid);
        const category = {
          id: item._data.id,
        ...
        this.categoriesaskList.push(category);
      });
    },
    handleGroupResource(res) {
      ...
      this.groupscontent = data.content;
      this.groupsurl = data.url;
    ...
    signUpSet(res) {
      ...
      this.pubFunc['createThread.14.disabled']=siteData.site_create_thread14===0;
    },
    // 扩展项回显
    setSelectValue(data) {
    ...
    /*提问*/
    getCategoriesask() {
      return this.appFetch({ url: "categories", method: "get", data: {'type': 7} });
    },
    getGroupResource() {
    ...
    checkSelect() {
      ...
      if (this.checked.indexOf('switch.createaskThread') !== -1) {
        if(this.selectList.createaskThread.length === 0){
          this.$message.error("请选择发布提问主题权限");
          return false;
        }
      }
      /*if (this.checked.indexOf('switch.thread.ask') !== -1) {
        if(this.selectList['thread.ask'].length === 0){
          this.$message.error("请选择发布提问权限");
          return false;
        }
      }*/
      ...
      if (this.checked.indexOf('switch.thread.askreply') !== -1) {
        if(this.selectList['thread.askreply'].length === 0){
          this.$message.error("请选择回复提问主题权限");
          return false;
        }
      }
      ...
      if (this.checked.indexOf('switch.thread.canaskBeReward') !== -1) {
        if(this.selectList['thread.canaskBeReward'].length === 0){
          this.$message.error("请选择允许提问被打赏权限");
          return false;
        }
      }
      ...
      if (this.checked.indexOf('switch.viewaskThreads') !== -1) {
        if(this.selectList.viewaskThreads.length === 0){
          this.$message.error("请选择查看提问主题列表权限");
          return false;
        }
      }
      ...
      if (this.checked.indexOf('switch.thread.viewaskPosts') !== -1) {
        if(this.selectList['thread.viewaskPosts'].length === 0){
          this.$message.error("请选择查看提问主题详情权限");
          return false;
        }
      }
      ...
      // 提问
      if (this.checked.indexOf('switch.thread.freeaskViewPosts.1') !== -1) {
        if(this.selectList['thread.freeaskViewPosts.1'].length === 0){
          this.$message.error("请选择免费查看提问付费帖子权限");
          return false;
        }
      }
      if (this.checked.indexOf('switch.thread.freeaskViewPosts.2') !== -1) {
        if(this.selectList['thread.freeaskViewPosts.2'].length === 0){
          this.$message.error("请选择免费查看提问付费视频权限");
          return false;
        }
      }
      if (this.checked.indexOf('switch.thread.freeaskViewPosts.3') !== -1) {
        if(this.selectList['thread.freeaskViewPosts.3'].length === 0){
          this.$message.error("请选择免费查看提问付费图片权限");
          return false;
        }
      }
      if (this.checked.indexOf('switch.thread.freeaskViewPosts.4') !== -1) {
        if(this.selectList['thread.freeaskViewPosts.4'].length === 0){
          this.$message.error("请选择免费查看提问付费语音权限");
          return false;
        }
      }
      if (this.checked.indexOf('switch.thread.freeaskViewPosts.5') !== -1) {
        if(this.selectList['thread.freeaskViewPosts.5'].length === 0){
          this.$message.error("请选择免费查看提问付费问答权限");
          return false;
        }
      }
      // 提问
      ...
      if (this.checked.indexOf('switch.thread.editOriginalThreadOrPost') !== -1) {
        if(this.selectList['thread.editOriginalThreadOrPost'].length === 0){
          this.$message.error("请选择编辑自己的主题为原创");
          return false;
        }
      }
      ...
      // 提问
      if (this.checked.indexOf('switch.thread.askessence') !== -1) {
        if(this.selectList['thread.askessence'].length === 0){
          this.$message.error("请选择加精权限");
          return false;
        }
      }
      if (this.checked.indexOf('switch.thread.askedit') !== -1) {
        if(this.selectList['thread.askedit'].length === 0){
          this.$message.error("请选择编辑主题权限");
          return false;
        }
      }
      if (this.checked.indexOf('switch.thread.askhide') !== -1) {
        if(this.selectList['thread.askhide'].length === 0){
          this.$message.error("请选择删除主题权限");
          return false;
        }
      }
      if (this.checked.indexOf('switch.thread.editaskPosts') !== -1) {
        if(this.selectList['thread.editaskPosts'].length === 0){
          this.$message.error("请选择编辑回复权限");
          return false;
        }
      }
      if (this.checked.indexOf('switch.thread.hideaskPosts') !== -1) {
        if(this.selectList['thread.hideaskPosts'].length === 0){
          this.$message.error("请选择删除回复权限");
          return false;
        }
      }
      if (this.checked.indexOf('switch.thread.editaskOwnThreadOrPost') !== -1) {
        if(this.selectList['thread.editaskOwnThreadOrPost'].length === 0){
          this.$message.error("请选择编辑自己的主题或回复权限");
          return false;
        }
      }
      if (this.checked.indexOf('switch.thread.hideaskOwnThreadOrPost') !== -1) {
        if(this.selectList['thread.hideaskOwnThreadOrPost'].length === 0){
          this.$message.error("请选择删除自己的主题或回复权限");
          return false;
        }
      }
      ...
      if (this.selectList[val.slice(0, 14)].includes(val)) {
        ...
        if (val.includes('position') || val.includes('anonymous')) {
          ...
          this.$confirm(str, '提示', {
            ...
            this.selectList[val.slice(0, 50)] =
              this.selectList[val.slice(0, 50)].filter(item => item !== val);
          });
        }
      } else {
    ...
    handleCheckAllChange(val) {
      ...
      this.selectList = {
        ...
        'viewaskThreads': [],
        'createThread':[], // 发布主题扩展项
        'createaskThread':[], // 发布提问主题扩展项
        'thread.ask':[], // 发布提问展项
        'thread.reply':[], // 回复主题扩展项
        'thread.askreply':[], // 回复主题扩展项
        'thread.canBeReward': [], //打赏扩展
        'thread.canaskBeReward': [], //打赏提问扩展
        'thread.viewPosts':[],
        'thread.viewaskPosts':[],
        'thread.edit':[], //
        'thread.hide':[], //
        ...
        'thread.editaskPosts':[], //
        'thread.hideaskPosts':[], //
        'thread.editaskOwnThreadOrPost': [], //
        'thread.hideaskOwnThreadOrPost': [], //
        'thread.freeViewPosts.1':[],
        'thread.freeViewPosts.2':[],
        'thread.freeViewPosts.3':[],
        'thread.freeViewPosts.4':[],
  ...
  created() {
    ...
    if (this.groupId === '7') {
      ...
      this.checkAllPermission = [
        ...
        "switch.viewaskThreads", //查看提问主题列表
        "switch.thread.viewPosts", //查看主题详情
        "switch.thread.viewaskPosts", //查看主题详情
        ...
        "switch.thread.freeaskViewPosts.1", //免费查看提问付费帖子
        "switch.thread.freeaskViewPosts.2", //免费查看提问付费视频
        "switch.thread.freeaskViewPosts.3", //免费查看提问付费图片
        "switch.thread.freeaskViewPosts.4", //免费查看提问付费语音
        "switch.thread.freeaskViewPosts.5", //免费查看提问付费问答
      ];
    } else {
      this.checkAllPermission = [
        ...
        "switch.createaskThread", //发布提问主题
        "switch.thread.ask", //发布提问主题
        "switch.thread.reply", //回复主题
        "switch.thread.askreply", //回复提问主题
        "switch.thread.canBeReward", //允许被打赏
        "switch.thread.canaskBeReward", //允许提问被打赏
        "switch.viewThreads", //查看主题列表
        "switch.viewaskThreads", //查看提问主题列表
        "switch.thread.viewPosts", //查看主题详情
        "switch.thread.viewaskPosts", //查看提问主题详情
        "switch.thread.freeViewPosts.1", //免费查看付费帖子
        "switch.thread.freeViewPosts.2", //免费查看付费视频
        "switch.thread.freeViewPosts.3", //免费查看付费图片
        "switch.thread.freeViewPosts.4", //免费查看付费语音
        ...
        "switch.thread.editOriginalThreadOrPost", //编辑自己的主题为原创
        "switch.thread.hideOwnThreadOrPost", //删除自己的主题或回复
        "switch.thread.askessence", //提问加精
        "switch.thread.askedit", //提问编辑主题
        "switch.thread.askhide", //提问删除主题
        "switch.thread.editaskPosts", //提问编辑回复
        "switch.thread.hideaskPosts", //提问删除回复
        "switch.thread.editaskOwnThreadOrPost", //提问编辑自己的主题或回复
        "switch.thread.hideaskOwnThreadOrPost", //提问删除自己的主题或回复
      ];
    }
  },
  components: {
