  ...
  <div class="rol-permission-box">
    ...
    <div v-show="activeTab.name === 'userOperate'">
      ...
      <div class="user-operate">
        ...
        <Card>
          <CardRow description="前台海报、取消海报主题的权限">
            <el-checkbox
              v-model="checked"
              label="thread.poster"
              :disabled="
        ...
        <Card>
          <CardRow description="前台评论、取消评论主题的权限">
            <el-checkbox
              v-model="checked"
              label="thread.pinglun"
              :disabled="
                $router.history.current.query.id === '1' ||
                  $router.history.current.query.id === '7'
              "
        ...
        <Card>
          <CardRow description="前台原创、取消原创主题的权限">
            <el-checkbox
              v-model="checked"
              label="thread.original"
              :disabled="
                $router.history.current.query.id === '1' ||
                  $router.history.current.query.id === '7'
              "
        ...
        <Card>
          <CardRow description="前台刷新主题的权限">
            <el-checkbox
              v-model="checked"
              label="thread.zhiding"
              :disabled="
                $router.history.current.query.id === '1' ||
                  $router.history.current.query.id === '7'
              "
        ...
        <!--Card class="hasSelect">
          <CardRow description="作者设置自己的主题为原创的权限">
            <el-checkbox
              v-model="checked"
              @change="changeChecked($event,'thread.editOriginalThreadOrPost')"
              label="switch.thread.editOriginalThreadOrPost"
              :disabled="
                $router.history.current.query.id === '1' ||
                  $router.history.current.query.id === '7'
              "
              >编辑自己的主题为原创</el-checkbox
            >
          </CardRow>
          <el-cascader
            :key="keyValue"
            placeholder="全选"
            v-model="selectList['thread.editOriginalThreadOrPost']"
            :options="categoriesList"
            :disabled="checked.indexOf('switch.thread.editOriginalThreadOrPost') === -1"
            :props="{
              value: 'id',
              label: 'name',
              children: 'children',
            ...
            collapse-tags
            @change="changeCategory($event, 'thread.editOriginalThreadOrPost')"
            @remove-tag="clearItem($event, 'thread.editOriginalThreadOrPost')"
          ></el-cascader>
        </Card-->
        <Card class="hasSelect">
          <CardRow description="作者编辑自己的主题或回复,原创 的权限">
            <el-checkbox
              v-model="checked"
        ...
        <Card class="hasSelect">
          <CardRow description="作者删除自己的主题或回复, 原创 的权限">
            <el-checkbox
              v-model="checked"
    ...
    <!--提问-->
    <div v-show="activeTab.name === 'useraskOperate'">
      <div class="user-operate__title">
        <el-checkbox
          :indeterminate="isIndeterminate"
          v-model="checkAll"
      ...
      <div class="user-operate">
        ...
        <Card class="hasSelect">
          <CardRow description="允许发布提问帖">
            <el-checkbox
              v-model="checked"
              label="createThread.14"
              :disabled="
                pubFunc['createThread.14.disabled'] ||
                $router.history.current.query.id === '1' ||
                $router.history.current.query.id === '7' ||
                $router.history.current.query.id === '14'
              "
              @change="changePostChecked($event,'createThread.14')"
              >提问帖子</el-checkbox
            >
          </CardRow>
          <CardRow description="允许发布提问红包帖">
            <el-checkbox
              v-model="checked"
              label="createThread.14.redPacket"
              :disabled="
                pubFunc['createThread.14.redPacket'] ||
                $router.history.current.query.id === '1' ||
                $router.history.current.query.id === '7' ||
                $router.history.current.query.id === '14'
              "
              @change="changePostChecked($event,'createThread.14.redPacket')"
              >提问红包</el-checkbox
            >
          </CardRow>
          <CardRow description="允许发布提问位置帖">
            <el-checkbox
              v-model="checked"
              label="createThread.14.position"
              :disabled="
                pubFunc['createThread.14.position'] ||
                $router.history.current.query.id === '1' ||
                $router.history.current.query.id === '7' ||
                $router.history.current.query.id === '14'
              "
              @change="changePostChecked($event,'createThread.14.position')"
              >提问位置</el-checkbox
            >
          </CardRow>
          <CardRow description="允许发布提问匿名发布">
            <el-checkbox
              v-model="checked"
              label="createThread.14.anonymous"
              :disabled="
                pubFunc['createThread.14.anonymous'] ||
                $router.history.current.query.id === '1' ||
                $router.history.current.query.id === '7' ||
                $router.history.current.query.id === '14'
              "
              @change="changePostChecked($event,'createThread.14.anonymous')"
              >提问匿名发布</el-checkbox
            >
          </CardRow>
        </Card>
        ...
        -->
        <Card class="hasSelect">
            <CardRow description="允许在内容分类发布提问的权限">
              <el-checkbox
                v-model="checked"
                label="switch.createaskThread"
                @change="changeChecked($event,'createaskThread')"
                :disabled="
                  $router.history.current.query.id === '1' ||
                    $router.history.current.query.id === '7'
                "
            ...
            <el-cascader
              :key="keyValue"
              placeholder="全选"
              v-model="selectList.createaskThread"
              :options="categoriesaskList"
              :disabled="checked.indexOf('switch.createaskThread') === -1"
              :props="{
                value: 'id',
                label: 'name',
                children: 'children',
              ...
              collapse-tags
              @change="changeCategory($event, 'createaskThread')"
              @remove-tag="clearItem($event, 'createaskThread')"
            ></el-cascader>
          </Card>
          <Card class="hasSelect">
            <CardRow description="允许在内容分类回复提问的权限">
              <el-checkbox
                v-model="checked"
                label="switch.thread.askreply"
                 @change="changeChecked($event,'thread.askreply')"
                :disabled="
                  $router.history.current.query.id === '1' ||
                    $router.history.current.query.id === '7'
                "
            ...
            <el-cascader
              :key="keyValue"
              placeholder="全选"
              v-model="selectList['thread.askreply']"
              :options="categoriesaskList"
              :disabled="checked.indexOf('switch.thread.askreply') === -1"
              :props="{
                value: 'id',
                label: 'name',
                children: 'children',
              ...
              collapse-tags
              @change="changeCategory($event, 'thread.askreply')"
              @remove-tag="clearItem($event, 'thread.askreply')"
            ></el-cascader>
          </Card>
          <Card class="hasSelect">
            <CardRow description="内容分类下提问内容允许被打赏的权限">
              <el-checkbox
                v-model="checked"
                label="switch.thread.canaskBeReward"
                @change="changeChecked($event,'thread.canaskBeReward')"
                :disabled="
                  $router.history.current.query.id === '1' ||
                    $router.history.current.query.id === '7' ||
                    wechatPayment ||
            ...
            <el-cascader
              :key="keyValue"
              placeholder="全选"
              v-model="selectList['thread.canaskBeReward']"
              :options="categoriesaskList"
              :disabled="checked.indexOf('switch.thread.canaskBeReward') === -1"
              :props="{
                value: 'id',
                label: 'name',
                children: 'children',
              ...
              collapse-tags
              @change="changeCategory($event, 'thread.canaskBeReward')"
              @remove-tag="clearItem($event, 'thread.canaskBeReward')"
            ></el-cascader>
          </Card>
      </div>
      <div class="user-operate">
        ...
        <Card class="hasSelect">
          <CardRow description="查看内容分类主题列表的权限">
            <el-checkbox
              v-model="checked"
              label="switch.viewaskThreads"
              @change="changeChecked($event,'viewaskThreads')"
              :disabled="$router.history.current.query.id === '1'"
              >查看提问主题列表</el-checkbox
            >
          </CardRow>
          <el-cascader
            :key="keyValue"
            placeholder="全选"
            v-model="selectList.viewaskThreads"
            :options="categoriesaskList"
            :disabled="checked.indexOf('switch.viewaskThreads') === -1"
            :props="{
              value: 'id',
              label: 'name',
              children: 'children',
            ...
            collapse-tags
            @change="changeCategory($event, 'viewaskThreads')"
            @remove-tag="clearItem($event, 'viewaskThreads')"
          ></el-cascader>
        </Card>
        <Card class="hasSelect">
          <CardRow description="查看提问内容分类主题详情的权限">
            <el-checkbox
              v-model="checked"
              label="switch.thread.viewaskPosts"
              @change="changeChecked($event,'thread.viewaskPosts')"
              :disabled="$router.history.current.query.id === '1'"
              >查看提问主题详情</el-checkbox
            >
          </CardRow>
          <el-cascader
            :key="keyValue"
            placeholder="全选"
            v-model="selectList['thread.viewaskPosts']"
            :options="categoriesaskList"
            :disabled="checked.indexOf('switch.thread.viewaskPosts') === -1"
            :props="{
              value: 'id',
              label: 'name',
              children: 'children',
            ...
            collapse-tags
            @change="changeCategory($event, 'thread.viewaskPosts')"
            @remove-tag="clearItem($event, 'thread.viewaskPosts')"
          ></el-cascader>
        </Card>
        <Card class="hasSelect">
          <CardRow description="免费查看提问内容分类下的付费帖子">
            <el-checkbox
              v-model="checked"
              label="switch.thread.freeaskViewPosts.1"
              @change="changeChecked($event,'thread.freeaskViewPosts.1')"
              :disabled="$router.history.current.query.id === '1'"
              >免费查看提问付费帖子</el-checkbox
            >
          </CardRow>
          <el-cascader
            :key="keyValue"
            placeholder="全选"
            v-model="selectList['thread.freeaskViewPosts.1']"
            :options="categoriesaskList"
            :disabled="checked.indexOf('switch.thread.freeaskViewPosts.1') === -1"
            :props="{
              value: 'id',
              label: 'name',
              children: 'children',
            ...
            collapse-tags
            @change="changeCategory($event, 'thread.freeaskViewPosts.1')"
            @remove-tag="clearItem($event, 'thread.freeaskViewPosts.1')"
          ></el-cascader>
        </Card>
        <Card class="hasSelect">
          <CardRow description="免费查看内容分类下的付费视频">
            <el-checkbox
              v-model="checked"
              label="switch.thread.freeaskViewPosts.2"
              @change="changeChecked($event,'thread.freeaskViewPosts.2')"
              :disabled="$router.history.current.query.id === '1'"
              >免费查看付费视频</el-checkbox
            >
          </CardRow>
          <el-cascader
            :key="keyValue"
            placeholder="全选"
            v-model="selectList['thread.freeaskViewPosts.2']"
            :options="categoriesaskList"
            :disabled="checked.indexOf('switch.thread.freeaskViewPosts.2') === -1"
            :props="{
              value: 'id',
              label: 'name',
              children: 'children',
            ...
            collapse-tags
            @change="changeCategory($event, 'thread.freeaskViewPosts.2')"
            @remove-tag="clearItem($event, 'thread.freeaskViewPosts.2')"
          ></el-cascader>
        </Card>
        <Card class="hasSelect">
          <CardRow description="免费查看内容分类下的付费图片">
            <el-checkbox
              v-model="checked"
              label="switch.thread.freeaskViewPosts.3"
              @change="changeChecked($event,'thread.freeaskViewPosts.3')"
              :disabled="$router.history.current.query.id === '1'"
              >免费查看付费图片</el-checkbox
            >
          </CardRow>
          <el-cascader
            :key="keyValue"
            placeholder="全选"
            v-model="selectList['thread.freeaskViewPosts.3']"
            :options="categoriesaskList"
            :disabled="checked.indexOf('switch.thread.freeaskViewPosts.3') === -1"
            :props="{
              value: 'id',
              label: 'name',
              children: 'children',
            ...
            collapse-tags
            @change="changeCategory($event, 'thread.freeaskViewPosts.3')"
            @remove-tag="clearItem($event, 'thread.freeaskViewPosts.3')"
          ></el-cascader>
        </Card>
        <Card class="hasSelect">
          <CardRow description="免费查看内容分类下的付费语音">
            <el-checkbox
              v-model="checked"
              label="switch.thread.freeaskViewPosts.4"
              @change="changeChecked($event,'thread.freeaskViewPosts.4')"
              :disabled="$router.history.current.query.id === '1'"
              >免费查看付费语音</el-checkbox
            >
          </CardRow>
          <el-cascader
            :key="keyValue"
            placeholder="全选"
            v-model="selectList['thread.freeaskViewPosts.4']"
            :options="categoriesaskList"
            :disabled="checked.indexOf('switch.thread.freeaskViewPosts.4') === -1"
            :props="{
              value: 'id',
              label: 'name',
              children: 'children',
            ...
            collapse-tags
            @change="changeCategory($event, 'thread.freeaskViewPosts.4')"
            @remove-tag="clearItem($event, 'thread.freeaskViewPosts.4')"
          ></el-cascader>
        </Card>
        <Card class="hasSelect">
          <CardRow description="免费查看内容分类下的付费问答">
            <el-checkbox
              v-model="checked"
              label="switch.thread.freeaskViewPosts.5"
              @change="changeChecked($event,'thread.freeaskViewPosts.5')"
              :disabled="$router.history.current.query.id === '1'"
              >免费查看付费问答</el-checkbox
            >
          </CardRow>
          <el-cascader
            :key="keyValue"
            placeholder="全选"
            v-model="selectList['thread.freeaskViewPosts.5']"
            :options="categoriesaskList"
            :disabled="checked.indexOf('switch.thread.freeaskViewPosts.5') === -1"
            :props="{
              value: 'id',
              label: 'name',
              children: 'children',
            ...
            collapse-tags
            @change="changeCategory($event, 'thread.freeaskViewPosts.5')"
            @remove-tag="clearItem($event, 'thread.freeaskViewPosts.5')"
          ></el-cascader>
        </Card>
      </div>
      <div class="user-operate">
        ...
        <Card class="hasSelect">
          <CardRow description="提问前台精华、取消精华主题的权限">
            <el-checkbox
              v-model="checked"
              label="switch.thread.askessence"
              @change="changeChecked($event,'thread.askessence')"
              :disabled="
                $router.history.current.query.id === '1' ||
                  $router.history.current.query.id === '7'
              "
          ...
          <el-cascader
            :key="keyValue"
            placeholder="全选"
            v-model="selectList['thread.askessence']"
            :options="categoriesaskList"
            :disabled="checked.indexOf('switch.thread.askessence') === -1"
            :props="{
              value: 'id',
              label: 'name',
              children: 'children',
            ...
            collapse-tags
            @change="changeCategory($event, 'thread.askessence')"
            @remove-tag="clearItem($event, 'thread.askessence')"
          ></el-cascader>
        </Card>
        <Card class="hasSelect">
          <CardRow description="前台单个主题的编辑权限">
          ...
          <el-cascader
            :key="keyValue"
            placeholder="全选"
            v-model="selectList['thread.askedit']"
            :options="categoriesaskList"
            :disabled="checked.indexOf('switch.thread.askedit') === -1"
            :props="{
              value: 'id',
              label: 'name',
              children: 'children',
            ...
            collapse-tags
            @change="changeCategory($event, 'thread.askedit')"
            @remove-tag="clearItem($event, 'thread.askedit')"
          ></el-cascader>
        </Card>
        <Card class="hasSelect">
          <CardRow description="前台删除单个主题的权限">
            <el-checkbox
              v-model="checked"
              label="switch.thread.askhide"
              @change="changeChecked($event,'thread.askhide')"
              :disabled="
                $router.history.current.query.id === '1' ||
                  $router.history.current.query.id === '7'
              "
          ...
          <el-cascader
            :key="keyValue"
            placeholder="全选"
            v-model="selectList['thread.askhide']"
            :options="categoriesaskList"
            :disabled="checked.indexOf('switch.thread.askhide') === -1"
            :props="{
              value: 'id',
              label: 'name',
              children: 'children',
            ...
            collapse-tags
            @change="changeCategory($event, 'thread.askhide')"
            @remove-tag="clearItem($event, 'thread.askhide')"
          ></el-cascader>
        </Card>
        <Card class="hasSelect">
          <CardRow description="前台单个回复的编辑权限">
            <el-checkbox
              v-model="checked"
              label="switch.thread.editaskPosts"
              @change="changeChecked($event,'thread.editaskPosts')"
              :disabled="
                $router.history.current.query.id === '1' ||
                  $router.history.current.query.id === '7'
              "
          ...
          <el-cascader
            :key="keyValue"
            placeholder="全选"
            v-model="selectList['thread.editaskPosts']"
            :options="categoriesaskList"
            :disabled="checked.indexOf('switch.thread.editaskPosts') === -1"
            :props="{
              value: 'id',
              label: 'name',
              children: 'children',
            ...
            collapse-tags
            @change="changeCategory($event, 'thread.editaskPosts')"
            @remove-tag="clearItem($event, 'thread.editaskPosts')"
          ></el-cascader>
        </Card>
        <Card class="hasSelect">
          <CardRow description="前台删除单个回复的权限">
            <el-checkbox
              v-model="checked"
              label="switch.thread.hideaskPosts"
              @change="changeChecked($event,'thread.hideaskPosts')"
              :disabled="
                $router.history.current.query.id === '1' ||
                  $router.history.current.query.id === '7'
              "
          ...
          <el-cascader
            :key="keyValue"
            placeholder="全选"
            v-model="selectList['thread.hideaskPosts']"
            :options="categoriesaskList"
            :disabled="checked.indexOf('switch.thread.hideaskPosts') === -1"
            :props="{
              value: 'id',
              label: 'name',
              children: 'children',
            ...
            collapse-tags
            @change="changeCategory($event, 'thread.hideaskPosts')"
            @remove-tag="clearItem($event, 'thread.hideaskPosts')"
          ></el-cascader>
        </Card>
        <Card class="hasSelect">
          <CardRow description="作者编辑自己的主题或回复的权限">
            <el-checkbox
              v-model="checked"
              @change="changeChecked($event,'thread.editaskOwnThreadOrPost')"
              label="switch.thread.editaskOwnThreadOrPost"
              :disabled="
                $router.history.current.query.id === '1' ||
                  $router.history.current.query.id === '7'
              "
          ...
          <el-cascader
            :key="keyValue"
            placeholder="全选"
            v-model="selectList['thread.editaskOwnThreadOrPost']"
            :options="categoriesaskList"
            :disabled="checked.indexOf('switch.thread.editaskOwnThreadOrPost') === -1"
            :props="{
              value: 'id',
              label: 'name',
              children: 'children',
            ...
            collapse-tags
            @change="changeCategory($event, 'thread.editaskOwnThreadOrPost')"
            @remove-tag="clearItem($event, 'thread.editaskOwnThreadOrPost')"
          ></el-cascader>
        </Card>
        <Card class="hasSelect">
          <CardRow description="作者删除自己的主题或回复的权限">
            <el-checkbox
              v-model="checked"
              @change="changeChecked($event,'thread.hideaskOwnThreadOrPost')"
              label="switch.thread.hideaskOwnThreadOrPost"
              :disabled="
                $router.history.current.query.id === '1' ||
                  $router.history.current.query.id === '7'
              "
          ...
          <el-cascader
            :key="keyValue"
            placeholder="全选"
            v-model="selectList['thread.hideaskOwnThreadOrPost']"
            :options="categoriesaskList"
            :disabled="checked.indexOf('switch.thread.hideaskOwnThreadOrPost') === -1"
            :props="{
              value: 'id',
              label: 'name',
              children: 'children',
            ...
            collapse-tags
            @change="changeCategory($event, 'thread.hideaskOwnThreadOrPost')"
            @remove-tag="clearItem($event, 'thread.hideaskOwnThreadOrPost')"
          ></el-cascader>
        </Card>
      </div>
    </div>
    ...
    <div v-show="activeTab.name === 'pricesetting'">
      ...
      <Card header="购买文章优惠：">
        <CardRow description="购买文章优惠(1~10),1折,2折,3折...">
          <el-input
            placeholder="购买文章优惠"
            type="number"
      ...
      <Card header="会员介绍：">
        <CardRow description="填写会员介绍">
          <el-input type="textarea" :autosize="{ minRows: 4, maxRows: 4 }" placeholder="会员介绍" v-model="groupscontent">
          </el-input>
        </CardRow>
      </Card>
      <Card header="跳转网址：">
        <CardRow description="填写网址">
          <el-input type="textarea" :autosize="{ minRows: 4, maxRows: 4 }" placeholder="网址" v-model="groupsurl">
          </el-input>
        </CardRow>
      </Card>
    </div>
