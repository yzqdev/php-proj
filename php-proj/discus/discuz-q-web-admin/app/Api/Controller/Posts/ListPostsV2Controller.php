...
class ListPostsV2Controller extends DzqController
{
    ...
    protected function search(array $filters, int $perPage, int $page, $sort)
    {
        ...
        $query = Post::query()
            ->with([
                'thread:id,type',
                'user:id,username,avatar,realname,last_login_ip as lastLoginIp,register_ip as registerIp',
    ...
    protected function getLastThreeComments($posts)
    {
        ...
        $allLastThreeComments = Post::query()
            ->with([
                'thread:id,type',
                'user:id,username,avatar,realname,last_login_ip as lastLoginIp,register_ip as registerIp',
                'commentUser:id,username,avatar,realname,last_login_ip as lastLoginIp,register_ip as registerIp',
                'replyUser:id,username,avatar,realname,last_login_ip as lastLoginIp,register_ip as registerIp',
                'images',
            ])
            ->from('posts', 'a')
            ->whereRaw('('.$subSql.') < ?', [3])
    ...
    protected function getPost(Post $post, bool $getRedPacketAmount)
    {
        ...
        if (!empty($ip) && isset($this->appcode)){
            $info = $this->Qqwry->getlocation($ip);
            $data['user']['province'] = $info;
        }


        $reip = $data['replyUser']['lastLoginIp'];
        if (empty($reip)){
            $reip = $data['replyUser']['registerIp'];
        }
        if (!empty($reip) && isset($this->appcode)){
            $reinfo = $this->Qqwry->getlocation($reip);
            $data['replyUser']['province'] = $reinfo;
        }

        $cUip = $data['commentUser']['lastLoginIp'];
        if (empty($cUip)){
            $cUip = $data['commentUser']['registerIp'];
        }
        if (!empty($cUip) && isset($this->appcode)){
            $cUinfo = $this->Qqwry->getlocation($cUip);
            $data['commentUser']['province'] = $cUinfo;
        }
