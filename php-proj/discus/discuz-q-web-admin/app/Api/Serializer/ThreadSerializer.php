...
class ThreadSerializer extends AbstractSerializer
{
    ...
    public function getDefaultAttributes($model)
    {
        ...
        //$arr = $this->Tools->RecontentHtml($this->getThreadContent($model->id),$this->getThreadContent($model->id));
        $attributes = [
            'id'                => $model->id,
            'type'              => (int) $model->type,
            'title'             => $model->title,
            ...
            'quanid'              => (int) $model->quanid, //圈子帖子id
            'keid'              => (int) $model->keid, //课程帖子id
            ...
            'isPoster'          => (bool) $model->is_poster, // 海报 2022-10-08 张安冠
            'isPinglun'         => (bool) $model->is_pinglun, // 评论管理 2022-11-08 张安冠
			'isOriginal'        => (bool) $model->is_original, // 原创管理 2022-11-18 张安冠
            ...
            'canPoster'         => $gate->allows('poster', $model),
            'canPinglun'         => $gate->allows('pinglun', $model),
			'canOriginal'         => $gate->allows('edit', $model),
            'canZhiding'         => $gate->allows('zhiding', $model),
