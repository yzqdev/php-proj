...
class ForumSettingSerializer extends AbstractSerializer
{
    ...
    public function getDefaultAttributes($model)
    {
        ...
        $gongzhonghaoLogo = $this->forumField->siteUrlSplicing($this->settings->get('gongzhonghao_logo'));
        $xiaochengxuLogo = $this->forumField->siteUrlSplicing($this->settings->get('xiaochengxu_logo'));
        $kefuyiLogo = $this->forumField->siteUrlSplicing($this->settings->get('kefuyi_logo'));
        $kefuerLogo = $this->forumField->siteUrlSplicing($this->settings->get('kefuer_logo'));
        $imga = $this->forumField->siteUrlSplicing($this->settings->get('imga'));
        $imgb = $this->forumField->siteUrlSplicing($this->settings->get('imgb'));
        $imgc = $this->forumField->siteUrlSplicing($this->settings->get('imgc'));
        $imgd = $this->forumField->siteUrlSplicing($this->settings->get('imgd'));
        $imge = $this->forumField->siteUrlSplicing($this->settings->get('imge'));
        
        $imgf = $this->forumField->siteUrlSplicing($this->settings->get('imgf'));
        $imgg = $this->forumField->siteUrlSplicing($this->settings->get('imgg'));
        $imgh = $this->forumField->siteUrlSplicing($this->settings->get('imgh'));
        $imgi = $this->forumField->siteUrlSplicing($this->settings->get('imgi'));
        $imgj = $this->forumField->siteUrlSplicing($this->settings->get('imgj'));
        
        $imgweia = $this->forumField->siteUrlSplicing($this->settings->get('imgweia'));
        $imgweib = $this->forumField->siteUrlSplicing($this->settings->get('imgweib'));
        $imgweic = $this->forumField->siteUrlSplicing($this->settings->get('imgweic'));
        $imgweid = $this->forumField->siteUrlSplicing($this->settings->get('imgweid'));
        $imgweie = $this->forumField->siteUrlSplicing($this->settings->get('imgweie'));
        
        $imgweif = $this->forumField->siteUrlSplicing($this->settings->get('imgweif'));
        $imgweig = $this->forumField->siteUrlSplicing($this->settings->get('imgweig'));
        $imgweih = $this->forumField->siteUrlSplicing($this->settings->get('imgweih'));
        $imgweii = $this->forumField->siteUrlSplicing($this->settings->get('imgweii'));
        $imgweij = $this->forumField->siteUrlSplicing($this->settings->get('imgweij'));
        ...
        $attributes = [
            ...
            'set_site' => [
                ...
                'site_keywords' => $this->settings->get('site_keywords11'),
                'site_introduction' => $this->settings->get('site_introduction'),
                'site_mode' => $this->settings->get('site_mode'), // pay public
                'site_weixinapp' => $this->settings->get('site_weixinapp'),
                'site_weixinkefu' => $this->settings->get('site_weixinkefu'),
                // 'site_ask' => $this->settings->get('site_ask'),
                'site_appcode' => $this->settings->get('site_appcode'),
                'site_ask' => json_decode($this->settings->get('site_ask'), true),
                'img_hangshu' => $this->settings->get('img_hangshu'),
                'img_wei_hangshu' => $this->settings->get('img_wei_hangshu'),
                'site_htmlapp' => json_decode($this->settings->get('site_htmlapp'), true),
                'site_weiapp' => json_decode($this->settings->get('site_weiapp'), true),
                'site_pinglun' => $this->settings->get('site_pinglun'),
                'site_pinglunzhiding' => $this->settings->get('site_pinglunzhiding'),
                'open_ext_fields'=>$this->settings->get('open_ext_fields'),
                'version' => 'v' . Application::VERSION,
                //'site_close' => (bool)$this->settings->get('site_close'),
                'site_growup' => json_decode($this->settings->get('site_growup'), true), //成长值 张安冠2022-10-1
                'site_growups' => json_decode($this->settings->get('site_growups'), true), //成长身份 张安冠2022-10-1
				'site_integral' => json_decode($this->settings->get('site_integral'), true), //积分 张安冠2022-10-1
                'site_signin' => json_decode($this->settings->get('site_signin'), true), //签到设置 张安冠2022-10-1
                'site_signinday' => json_decode($this->settings->get('site_signinday'), true), //签到天数 张安冠2022-10-1
                ...
                'site_gongzhonghao_logo' => $gongzhonghaoLogo ?: '',
                'site_xiaochengxu_logo' => $xiaochengxuLogo ?: '',
                'site_kefuyi_logo' => $kefuyiLogo ?: '',
                'site_kefuer_logo' => $kefuerLogo ?: '',
                'site_img0' => $imga ?: '',
                'site_img1' => $imgb ?: '',
                'site_img2' => $imgc ?: '',
                'site_img3' => $imgd ?: '',
                ...
                'site_img8' => $imgi ?: '',
                'site_img9' => $imgj ?: '',
                'site_wei_img0' => $imgweia ?: '',
                'site_wei_img1' => $imgweib ?: '',
                'site_wei_img2' => $imgweic ?: '',
                'site_wei_img3' => $imgweid ?: '',
                'site_wei_img4' => $imgweie ?: '',
                'site_wei_img5' => $imgweif ?: '',
                'site_wei_img6' => $imgweig ?: '',
                'site_wei_img7' => $imgweih ?: '',
                'site_wei_img8' => $imgweii ?: '',
                'site_wei_img9' => $imgweij ?: '',
                'site_url' => $siteUrl,
                ...
                'site_create_thread14' => $this->settings->get('site_create_thread14') == "" ? 1 : (int)$this->settings->get('site_create_thread14'),
            ...
            'paycenter' => [
                ...
                'wxpay_mchpay2_close' => (bool)$this->settings->get('wxpay_mchpay2_close', 'wxpay'),
            ],
            ...
            'other' => [
                ...
                'can_create_thread_ask' => $actor->can('createThread.' . Thread::TYPE_OF_ASK),              // 发布提问 2022-10-20 张安冠
                ...
                'can_create_thread_ask_position'      => $actor->can('createThread.' . Thread::TYPE_OF_ASK . '.position'),         // 发布提问位置
                ...
                'can_create_thread_ask_packet'          => $actor->can('createThread.' . Thread::TYPE_OF_ASK . '.redPacket'),     // 发提问帖红包
                ...
                'can_create_thread_ask_anonymous'     => $actor->can('createThread.' . Thread::TYPE_OF_ASK . '.anonymous'),     // 发布提问匿名发布
