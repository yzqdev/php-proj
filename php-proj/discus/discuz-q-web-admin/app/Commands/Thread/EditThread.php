...
class EditThread
{
    ...
    public function handle(Dispatcher $events, ThreadRepository $threads, Censor $censor, ThreadValidator $validator, ThreadVideoRepository $threadVideos, BusDispatcher $bus)
    {
        ...
        if (isset($attributes['isPoster'])) {
            ...
            if($this->actor->can('poster', $thread)){
        ...
        if (isset($attributes['isPinglun'])) {
            // $this->assertCan($this->actor, 'isSticky', $thread);

            if($this->actor->can('pinglun', $thread)){
                if ($thread->is_pinglun != $attributes['isPinglun']) {
                    $thread->is_pinglun = $attributes['isPinglun'];

                    if ($thread->is_pinglun) {
                        $this->threadNotices($thread, $this->actor, 'isPinglun', $attributes['message'] ?? '');
                    }
                }
            }
        }
        ...
        if (isset($attributes['isOriginal'])) {
            if($this->actor->can('Edit', $thread)){
                if ($thread->is_original != $attributes['isOriginal']) {
                    $thread->is_original = $attributes['isOriginal'];
                    if ($thread->is_original) {
                        $this->threadNotices($thread, $this->actor, 'isOriginal', $attributes['message'] ?? '');
                    }
                }
            }
        }
        ...
        if (isset($attributes['isZhiding'])) {
            $thread->updated_at = Carbon::now()->timestamp;
        }
