...
class UpdateGroup
{
    ...
    public function __invoke()
    {
        ...
        if (Arr::has($this->data, 'attributes.content')) {
            $group->content = Arr::get($this->data, 'attributes.content', '');
        }

        // 2022-11-20 张安冠 跳转链接

        if (Arr::has($this->data, 'attributes.url')) {
            $group->url = Arr::get($this->data, 'attributes.url', '');
        }
        
        // 2022-11-20 张安冠 优惠价格

        if (Arr::has($this->data, 'attributes.discount')) {
            $group->discount = Arr::get($this->data, 'attributes.discount', '');
        }
