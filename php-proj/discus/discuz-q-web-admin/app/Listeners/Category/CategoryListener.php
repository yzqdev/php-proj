    ...
    public function whenCategoryCreated(Created $event)
    {
        ...
        foreach ($ids as $key => $value) {
            if($value !== Group::GUEST_ID){
                ...
                //张安冠202210-30
				if(!in_array('createaskThread', $groupPermissions[$value])){
				    $newPermissions[] = [
				        'group_id' => $value,
				        'permission' => "category{$category->id}.createaskThread",
				    ];
				}
                ...
                if(!in_array('viewaskThreads', $groupPermissions[$value])){
                    $newPermissions[] = [
                        'group_id' => $value,
                        'permission' => "category{$category->id}.viewaskThreads",
                    ];
                }
                ...
                if(!in_array('thread.askreply', $groupPermissions[$value])){
                    $newPermissions[] = [
                        'group_id' => $value,
                        'permission' => "category{$category->id}.thread.askreply",
                    ];
                }
                ...
                if(!in_array('thread.viewaskPosts', $groupPermissions[$value])){
                    $newPermissions[] = [
                        'group_id' => $value,
                        'permission' => "category{$category->id}.thread.viewaskPosts",
                    ];
                }

                if(!in_array('thread.ask', $groupPermissions[$value])){
				    $newPermissions[] = [
				        'group_id' => $value,
				        'permission' => "category{$category->id}.thread.ask",
				    ];
				}
            }else{
