...
use App\Models\GroupUser; // 更新用户的用户组
...
class PaidGroupOrder
{
    public function handle(PaidGroup $event)
    {
        if (isset($event->group_id)) {
            ...
            //先删除老的 group_user
            $same = GroupUser::query()->where('user_id', $event->user->id)->get();
            //$same = json_decode($same);
            /*if($same['group_id'] !== $event->group_id) {
            ...
            /*else {
                //GroupUser::query()->insert(['group_id' => $event->group_id, 'user_id' => $event->user->id, 'expiration_time' => $expiration_time]);
                //$res = GroupUser::query()->where('user_id', $event->user->id)->delete();
            }*/
            /*$res = GroupUser::query()->where('user_id', $event->user->id)->delete();
                if ($res === false) {
            ...
            if (in_array($event->group_id, $user_group_ids)) {
                ...
                //同时更新用户的用户组
                //GroupUser::query()->where('user_id', $event->user->id)->update(['group_id' => $group_info->id, 'expiration_time' => $expiration_time]);
            } else {
                //删除原来用户组：
                //GroupUser::where('user_id', $event->user->id)->delete();
                //先删除老的 group_user
                $same = GroupUser::query()->where('user_id', $event->user->id)->get();
                if($same['group_id'] !== $event->group_id) {
                    $res = GroupUser::query()->where('user_id', $event->user->id)->delete();
